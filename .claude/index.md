...
| [error-handling.md](decisions/error-handling.md) | The single app-wide `ErrorBoundary` and its real (narrow) coverage; dual XML syntax checking; the clean-export fallback chain and its risky final tier; Commands' return-value-not-throw contract; the GitHub 409 conflict message; host-originated error toasts redirected to the Host Alerts panel at the API boundary, not by message length; the pre-ready `_q` queue's ordered-replay fix and `clearHostErrors()` resetting `requestedValidationTab`; the fixed toast string being operation-agnostic rather than "Failed generating program" |
...
| ...a host-reported error toast is short but the Host Alerts panel has the full detail | [error-handling.md](decisions/error-handling.md) #6 |
| ...the pre-ready host-API queue (`_q`) replays `showFeedback`/`showErrors`/`clearErrors` in the host's original call order, and clearing host errors also cancels a pending Host Alerts tab switch | [error-handling.md](decisions/error-handling.md) #7 |
| ...the host-reported error toast says "Host reported an error" rather than naming a specific operation like "generating program" | [error-handling.md](decisions/error-handling.md) #8 |
| ...there's no Playwright/Cypress in this repo | [testing.md](decisions/testing.md) #1 |
| ...only 10 test files (7 components + 3 hooks) use `@testing-library/react` | [testing.md](decisions/testing.md) #2 |

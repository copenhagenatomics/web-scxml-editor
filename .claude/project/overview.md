...
- **Downstream C# generator constraints that the editor must account for**: event names can't be C# reserved words or start with a digit, `assign/@location` can't contain a wildcard, and two event names can't sanitize to the same generated identifier. These are documented in `docs/invalid-event-identifiers.md`; several still lack automated validator coverage (see `.claude/project/scxml-rules.md`).

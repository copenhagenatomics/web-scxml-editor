...
### Decision
`visual-diagram.tsx` filters any onentry/onexit action string matching the timer-generated pattern (`isTimerGeneratedActionString`, `src/lib/utils/time-transition.ts`) out of what's passed to `StateActionsPanel` and out of what's passed to `SCXMLStateNode`'s action-count display. The filtered-out raw strings are retained together with their original index in the source array (`selectedStateForActions.hiddenEntryActions`/`hiddenExitActions: Array<{ index, action }>`) and merged back into whatever the panel commits (`onApply`, `onNewChannel`'s "new channel" path) via `mergeHiddenActions()`, which splices each hidden action back in at its recorded index (clamped to the edited list's current length) instead of appending at the end. So editing an unrelated action row never drops the timer's `send`/`cancel` elements from the document, *and* doesn't reorder them relative to the actions around them.
...
The original implementation re-appended hidden actions at the *end* of the edited list rather than at their original position. Because onentry/onexit actions execute in document order, this could silently change executable semantics — e.g. a `delayexpr` reading a variable's value as of a later point than originally authored, if an `assign` action that used to follow the hidden `send` now precedes it. Found via a Copilot PR review comment (2026-09-07); fixed by tracking each hidden action's original index and merging back positionally instead of by simple concatenation.

### Constraints
Any new site that reads a state's raw `entryActions`/`exitActions` for **display** purposes must apply the same filter, or a timer's rows will leak back into view there. Any site that **writes** a state's onentry/onexit from a UI-sourced action list (not just `StateActionsPanel`) must merge back any timer-generated rows it didn't include **at their original position**, or it risks both silently deleting the timer (if omitted entirely) and silently reordering it relative to surrounding actions (if naively appended) — see `mergeHiddenActions()` as the template. Index-based repositioning is best-effort, not exact, once the user has added or removed rows in the edited list (indices are clamped to the new length); it is exact when the edited list's row count is unchanged.

### Alternatives
None found evidenced for the original hide/re-append design. The positional merge-back (`mergeHiddenActions`, splicing at a clamped original index) replaced the original simple-concatenation merge-back after the ordering bug above was found — no other repositioning strategy was evaluated.

### Evidence
`src/lib/utils/time-transition.ts` (`isTimerGeneratedActionString`, `mergeHiddenActions`), `src/components/diagram/visual-diagram.tsx` (`parseActions` filter, `hiddenEntryActions`/`hiddenExitActions`, the `onApply`/`handleNewChannelForStateActions` merge-back), `src/components/diagram/nodes/scxml-state-node.tsx` (`visibleEntryActions`/`visibleExitActions`), `src/lib/utils/time-transition.test.ts` (`mergeHiddenActions` describe block).

...
- **Copy/cut/paste**: Ctrl/Cmd+C copies the selection (states clone with fresh ids, subtree included); Ctrl/Cmd+X cuts (copies, then deletes the selection); Ctrl/Cmd+V pastes with an escalating offset per repeated paste so pasted copies don't stack exactly on top of each other. A paste only appends a `_copy` suffix when the original id is still taken (an ordinary copy, where the source state still exists) — a cut-then-paste reuses the original id unchanged, since `cloneStateSubtreeWithFreshIds`'s `freshId` checks `existingIds` before manufacturing a suffix. `cloneStateSubtreeWithFreshIds` remaps a cloned compound state's own initial-child marking in **both** representations SCXML allows for it: the `@_initial` attribute and the legacy `<initial><transition target="X"/></initial>` child element (see `scxml.md` decision #4 — the element form isn't normalized away until the user explicitly toggles that state's Initial flag, so a copied/cut subtree can still carry it); missing either one left a pasted compound state's initial marking dangling at the old, no-longer-present id. The Multi-Select Toolbar exposes the same Copy/Cut/Delete actions as buttons.
- **Carrying "Initial" across a copy/cut + paste to a different level**: "Initial" is recorded on the *parent* container's own attribute (see `isMarkedInitial` in `initial-group-utils.ts`), not on the state element itself, so copying/cutting a state that was its old parent's Initial child and pasting it under a different parent doesn't automatically make it Initial there — the pasted clone's own data never carried that fact. `copyActiveStatesToClipboard` (in `visual-diagram.tsx`) additionally records, per top-level copied id, whether it was Initial in its source parent (`useStateClipboardStore`'s `copiedInitialIds`); `handlePasteClipboard` checks whether the *paste target* has no Initial marking of its own (`getInitialIds(...).size === 0`, checked **before** the new clones are added) and, if so, applies **every** such carried-over id (not just one) via `resolveCarriedOverInitialIds` (`scxml-manipulation-utils.ts`), joined as a space-separated `@_initial`. This mirrors — rather than reimplements — the existing "an empty container adopts its first dropped member as Initial for free" rule `handleReparent` already applies to drag-to-reparent. Pasting into a parent that already designates its own Initial state never overrides or merges into it (that requires the explicit, conflict-aware `ToggleInitialStateCommand` flow) — the carry-over only fires when the target had none.
  - **Parallel State copy/cut**: a Parallel State's regions are each Initial in their own independent work tree — that plurality is exactly what `parallel-group-normalization.ts`'s `normalizeParallelGroups` (run synchronously on every diagram-triggered `onSCXMLChange`, see `editor-store.ts`'s `normalizeContent`) uses to decide "these 2+ siblings should be wrapped in a real `<parallel>` element." Selecting and copying/cutting 2+ region members therefore needs **all** of their Initial markers reproduced on the pasted target, not just one — carrying over only the first would leave the rest as ordinary flat siblings, silently dropping the `<parallel>` wrapping the moment they're pasted. `resolveCarriedOverInitialIds` returning every formerly-Initial id (not the single first match an earlier version of this fix used) is what lets `normalizeParallelGroups` re-wrap the pasted copy into its own `<parallel>` on the very next content update, the same way it created the original.
...
- The selection toolbar (`MultiSelectToolbar`) appears for 1+ selected nodes (single selection included, despite the component's "multi-select" name), so Copy/Cut/Delete are reachable from a single-state selection too. Copy also clears the selection once it completes, matching Cut/Delete's existing behavior of closing the toolbar after their action runs.
- Copy/Cut/Delete each show a transient toast confirming the action (e.g. "State copied.", "3 states cut.", "State deleted.") via `useHostAPIStore`'s `showFeedback` — the same feedback-queue mechanism already used elsewhere in the app (GitHub push/pull, State Actions panel copy/paste, config deletion), rendered by `two-tab-layout.tsx` and auto-dismissed after 4s. `handleNodeDelete` is the single choke point for all deletes (keyboard Delete, per-node delete button, toolbar Delete), so its toast covers all of them; Cut copies via a toast-less internal helper (`copyActiveStatesToClipboard`) so a cut shows one "cut" toast rather than stacking a "copied" toast underneath it.
...
- The main "apply freshly re-parsed nodes/edges" resync effect and its `applyLatestEnhancedNodes` one-shot retry (see `visual-diagram.md` #14) used to gate on `enhancedNodes.length > 0`, which — besides skipping the pre-first-parse window it was meant for — also silently skipped applying a legitimate *empty* result, e.g. deleting every node at the current hierarchy level left the canvas showing the just-deleted nodes until an unrelated event forced a resync. Both gates now use `enhancedNodes.length > 0 || hasParsedOnceRef.current`, where `hasParsedOnceRef` is set once the first parse attempt (success or failure) completes, so a genuinely-empty level after that point still gets applied.

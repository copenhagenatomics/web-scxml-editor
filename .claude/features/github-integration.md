...
- Do not key `getValidAccessToken()`'s in-flight-refresh dedup by nothing (a single global promise), and do not let its completion handlers call `updateTokens()`/`clearAuth()` without first checking the store's current `refreshToken` still matches the one the request started with — either omission lets a stale refresh completion (from a session the user has since signed out of or reconnected past) silently corrupt or tear down the new session.

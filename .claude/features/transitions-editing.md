...
- **Fixed**: the A*-obstacle-avoidance path (`smartPath` in `scxml-transition-edge.tsx`, used when a nearby state blocks a transition's direct route) used to place its label using `@tisoap/react-flow-smart-edge`'s own `edgeCenterX/edgeCenterY`, which that library computes from its raw, unsimplified A* grid walk (`fullPath`) rather than the simplified/rounded path it actually draws (`smoothedPath`/`graphPath`). Those two point lists have unrelated densities per leg, so the label could land well off the rendered line — reported as a transition label "floating away from the transition," specifically after dragging a state near an existing edge. Fixed by capturing the real midpoint from the same points the line is drawn through (`getOrthogonalPathMidpoint` in `path-builders.ts`) instead of trusting the library's value.
- **Fixed**: for that same A*-obstacle-avoidance path, nothing constrained the grid search's final hop to arrive from the side the target handle is actually on — `computeOrthogonalCorners` only guaranteed every segment was *some* axis-aligned direction, not the *correct* one, so a route could legitimately approach a `top`/`bottom` handle horizontally (or a `left`/`right` handle vertically). Since the arrowhead `<marker orient="auto">` rotates strictly from the final segment's tangent, this rendered as an arrow pointing sideways into the node instead of through its handle — reported as "incorrect transition arrow direction." Fixed in `path-builders.ts` (the `targetSide` parameter threaded through `buildRoundedOrthogonalPath`/`getOrthogonalPathMidpoint`) by sliding the *earlier* corner so the turn toward the target happens there instead of at the last point — reusing distance the route already covers rather than inserting a new detour. (An earlier version of this fix always inserted a fresh perpendicular stub past the last corner; since that corner is always already level with the target on the wrong axis — same orthogonality invariant — a fixed-size stub made the line double back on itself, visible as a stray loop right at the arrowhead. The stub is now only a fallback for the rare case with no earlier corner to reuse.)

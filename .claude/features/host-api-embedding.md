...
- A host `showFeedback(message, 'error')` call never renders `message` verbatim in the toast: the toast is replaced with a fixed, operation-agnostic short string ("Host reported an error. See Error Panel for details.") and the full `message` is pushed into the Host Alerts panel instead — see "Internal architecture" below and `decisions/error-handling.md` #6, #8. Non-error host feedback (`'info'`/`'warning'`) is unaffected. This repo's own internal `showFeedback('error', ...)` toasts (GitHub push/pull, panel saves, etc.) bypass this entirely — they're already short and call the store directly.
...
- **Pre-ready queue pattern (`_q`)**: if the host's own script runs and calls `window.ScxmlEditorAPI` methods *before* this app's React tree has mounted, those calls are stashed on `window.ScxmlEditorAPI._q` (a plain object with `ready`/`commands`/`ops`/`channels`/`channelMappings`/`events` fields). `showFeedback`/`showErrors`/`clearErrors` calls each push one typed entry onto the single `ops` array (`{type:'feedback',...}` / `{type:'showErrors',...}` / `{type:'clearErrors'}`) rather than three separate buckets, specifically so they can be replayed in the host's actual original call order — see `decisions/error-handling.md` #7. Once the real API is ready, `use-host-api-bridge.ts` **upgrades the stub object in place** (`Object.assign(stub, realApi)`, then `delete stub._q`) — this means any reference the host already captured (e.g. `var api = iframe.contentWindow.ScxmlEditorAPI` grabbed early) automatically gains the real methods without needing to re-read the property, and `queue.ops` is replayed in array order.
  - **The stub script's source of truth is `src/lib/host-api/pre-ready-stub.ts`** (`PRE_READY_STUB_SCRIPT`, a plain JS string), embedded into the page by `src/app/layout.tsx` via a `<script dangerouslySetInnerHTML>` (in the `<body>`, before `{children}`) that runs synchronously on page load, before React hydrates: `if (window.ScxmlEditorAPI) return;` (never overwrites a stub the host may have injected even earlier) then defines `window.ScxmlEditorAPI = {_q, onReady, registerCommand, showFeedback, setChannels, showErrors, clearErrors, loadScxml, getScxml, toggleConfigPanel, setActiveTab}` — note this inline stub only pre-declares a **subset** of the full `ScxmlEditorAPI` surface (e.g. no `setConfigValues`/`getChannelMappings`/`setChannelMappings`/`setEvents`/`getEvents`/`toggleChannelMappingPanel`/`toggleEventsPanel` stubs exist here) — a host calling one of the *unstubbed* methods before React mounts would hit `undefined is not a function`, not a queued no-op. This script is a second, independent implementation of the "same idea" as `use-host-api-bridge.ts`'s real API — the two must be kept conceptually in sync by hand; there is no shared type-checked contract between the literal JS string in `pre-ready-stub.ts` and the real `ScxmlEditorAPI` TypeScript interface. `use-host-api-bridge.test.ts` does execute this exact script (via `new Function(PRE_READY_STUB_SCRIPT)`) and drive it through its real `showFeedback`/`showErrors`/`clearErrors` methods in some ordering tests, so a shape mismatch between what the stub pushes onto `_q.ops` and what `use-host-api-bridge.ts`'s drain logic expects would fail those tests — but this is call-shape coverage, not a type-checked contract, and the other hand-constructed `_q.ops` ordering tests still bypass the stub entirely.
...
`src/types/host-api.ts`, `src/app/_hooks/use-host-api-bridge.ts`, `src/app/_hooks/use-initial-load.ts`, `src/stores/host-api-store.ts`, `src/lib/host-api/pre-ready-stub.ts` (the pre-ready `_q` stub script, embedded by `src/app/layout.tsx`).

## Tests

`use-host-api-bridge.test.ts` covers the `hostShowFeedback` sanitization behavior (both the live `realApi.showFeedback` path and the pre-ready `_q.ops` replay) and the ops-ordering guarantee for all three `QueuedOp` variants queued before mount: `clearErrors`-then-error-`feedback` and the reverse, plus `clearErrors`-then-explicit-`showErrors` and the reverse. Most of these hand-construct the `_q.ops` array to test the bridge's drain logic in isolation; two of them (`clearErrors`-then-`feedback` and `clearErrors`-then-`showErrors`) additionally run through `installRealPreReadyStub()`, which executes the actual `PRE_READY_STUB_SCRIPT` (via `new Function`) and calls its real `showFeedback`/`showErrors`/`clearErrors` methods — so a shape mismatch between what `pre-ready-stub.ts` pushes onto `_q.ops` and what the bridge's drain logic expects would fail those two tests specifically, not just the hand-constructed ones. Not covered: the rest of the hook (`loadScxml`, `getScxml`/`getChannelMappings`, `markReady`/`onReady` ordering). `host-api-store.test.ts` covers `clearHostErrors()` resetting `requestedValidationTab` alongside `hostErrors`, and the non-`Error` throw normalization in `executeCommand`'s catch block. `use-initial-load.ts` still has no dedicated test file. This remains a partial gap given this is the primary production integration surface (per `.claude/project/overview.md`, embedded-in-LoopControl is the primary deployment mode, not standalone).

## Known limitations

- **Confirmed gap**: the pre-init stub script (`src/lib/host-api/pre-ready-stub.ts`, embedded by `layout.tsx`) only pre-declares `onReady`/`registerCommand`/`showFeedback`/`setChannels`/`showErrors`/`clearErrors`/`loadScxml`/`getScxml`/`toggleConfigPanel`/`setActiveTab`. A host calling `getConfigValues`, `setConfigValues`, `getChannelMappings`, `setChannelMappings`, `toggleChannelMappingPanel`, `setEvents`, `getEvents`, or `toggleEventsPanel` **before** this app's React tree mounts would hit a `TypeError` (method doesn't exist on the stub), not a silently-queued no-op like the other methods get. If a host integration ever needs to call one of these methods immediately on iframe load (rather than inside an `onReady` callback, which is always safe), the stub in `pre-ready-stub.ts` would need to be extended to match.
- `_q` pre-ready queue ordering (`showFeedback`/`showErrors`/`clearErrors` interleaving) has test coverage now (`use-host-api-bridge.test.ts`), but the stub-upgrade mechanism itself (`Object.assign(stub, realApi)`, `markReady`/`onReady` ordering) is still untested.
...
- Do not assign the store's `showFeedback` directly to `realApi.showFeedback` or the `_q.ops` replay — always route through `hostShowFeedback` (see "Internal architecture"), or host-originated `'error'` toasts will render raw exception text again.
...
- Do not split `_q`'s pre-ready calls back into per-method buckets (a `feedback` array, a separate `hostErrors` array, a `clearErrors` flag) — that structurally cannot preserve the host's actual call order across methods; see `decisions/error-handling.md` #7.
- Do not clear `hostErrors` without also resetting `requestedValidationTab` (or vice versa) — `clearHostErrors()` must keep doing both, or a stale tab-switch request force-opens Host Alerts on an empty error list; see `decisions/error-handling.md` #7.

## Previous design decisions

`docs/superpowers/plans/2026-05-21-loopcontrol-init-behavior.md` documents the origin of the embedded-detection-deferred-to-`useEffect` fix and the 3-second host-load timeout — implying an earlier version had the Welcome-screen-flash bug this now avoids. `docs/superpowers/plans/2026-05-27-host-error-panel.md` documents the "Host Alerts" tab as a later, deliberately-separate-from-validation addition (not an extension of the existing Validation Panel's error list) — a decision that speaks to keeping "this editor's own opinion about the file" architecturally distinct from "what the host is telling the user." `decisions/error-handling.md` #6 documents the later addition that bridges the two: a host-originated error-level *toast* is now automatically mirrored into that same Host Alerts channel, sanitized at the `window.ScxmlEditorAPI` boundary rather than by message content. `decisions/error-handling.md` #7 documents the follow-up fix: the pre-ready `_q` queue replays calls in the host's actual order (a single `ops` list, not per-method buckets), and `clearHostErrors()` resets `requestedValidationTab` alongside `hostErrors`.
